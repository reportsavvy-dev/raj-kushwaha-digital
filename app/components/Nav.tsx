...
export function Nav() {
  ...
  return <header className="nav shell">
    <Link href="/" prefetch={false} onClick={closeAll}><Logo/></Link>
    ...
    <nav id="primary-navigation" className={open ? "open" : ""}>
      <Link href="/" prefetch={false} onClick={closeAll}>HOME</Link>
      <Link href="/work" prefetch={false} onClick={closeAll}>WORK</Link>
      <Link href="/insights" prefetch={false} onClick={closeAll}>INSIGHTS</Link>
      <div className={`expertise-nav ${expertiseOpen ? "is-open" : ""}`} onMouseLeave={() => setExpertiseOpen(false)}>
        <div className="expertise-nav-trigger">
          <Link href="/expertise" prefetch={false} onMouseEnter={() => setExpertiseOpen(true)} onFocus={() => setExpertiseOpen(true)} onClick={() => setOpen(false)}>EXPERTISE</Link>
        ...
        <div className="expertise-dropdown" aria-label="All services">
          ...
          <div>{serviceIndex.map((service, index) => <Link href={`/services/${service.slug}`} prefetch={false} key={service.slug} onClick={closeAll}>
      ...
      <Link href="/contact" prefetch={false} onClick={closeAll}>CONTACT</Link>
    </nav>
  </header>;
}
